...
import { GlobalNotesStore, NoteEntry, NotesStore } from './types';
import { codeBlockMarkdown, escapeMarkdown, findAnchorCandidates, formatTimestamp, generateUUID, getGitCommitHash, getGitUserName, priorityBadge, prioritySort, timeAgo } from './utils';


export class NotesManager implements vscode.Disposable, INotesBrowserManager {
  ...
  private initialize(): void {
    ...
    this.notesFilePath = ws
      ? path.join(ws, '.vscode', 'note-stack.json')
      : '';
    ...
    this.globalNotesFilePath = path.join(globalDir, `note-stack-${APP_ID}.json`);
    ...
    this.treeDataProvider = new NotesTreeDataProvider(
      () => this.notes,
      (item) => { this.openNote(item); }
    );
    vscode.window.createTreeView('noteStack', { treeDataProvider: this.treeDataProvider });
  ...
  private getGutterIcon(color: string): string {
    ...
    try { fs.writeFileSync(iconPath, svg); } catch (e) { this.log(`Icon write failed: ${e}`); }
    return iconPath;
  }
  ...
  private getRelativeFilePath(fsPath: string): string {
    ...
    return root && fsPath.startsWith(root) ? path.relative(root, fsPath) : fsPath;
  }
  ...
  private setupEventListeners(): void {
    vscode.workspace.onDidChangeConfiguration(e => {
      if (e.affectsConfiguration('codeNotes.ticketTrackers') && this.browserPanel) {
      ...
      if (e.affectsConfiguration('noteStack.storageLocation')) {
        ...
        this.globalNotesFilePath = newPath
          ...
          : path.join(this.context.globalStorageUri.fsPath, `note-stack-${APP_ID}.json`);
      ...
      if (editor) {
        setTimeout(() => {
          ...
          if (this.browserPanel) {
            try { this.browserPanel.update(this); } catch { /* panel may be disposing */ }
          }
        }, 100);
      }
    });
  ...
  // ── Persistence ────────────────────────────────────────────────────────────
   ...
   private readAllGlobalStores(): GlobalNotesStore {
    ...
    try {
      ...
      const files = fs.readdirSync(dir).filter(f => f.startsWith('note-stack-') && f.endsWith('.json'));
        ...
        try {
          ...
          for (const [wsKey, wsStore] of Object.entries(store)) {
            ...
            for (const [filePath, notes] of Object.entries(wsStore)) {
              ...
              */
              const visible = notes.filter(n => !n.private || n.machineId?.startsWith(APP_ID))
  ...
  private readMyGlobalStore(): GlobalNotesStore {
    ...
    try {
      ...
      vscode.window.showErrorMessage(`NoteStack: Notes file is corrupt: ${this.globalNotesFilePath}`);
      return {};
    }
  }
  ...
  private async loadNotes(): Promise<void> {
    ...
    if (store[key] && Object.keys(store[key]).length > 0) {
      ...
      this.log(`Loaded ${Object.keys(this.notes).length} files from store (key: ${key})`);
    ...
    if (this.notesFilePath && fs.existsSync(this.notesFilePath)) {
      try {
        const local = JSON.parse(fs.readFileSync(this.notesFilePath, 'utf8')) as NotesStore;
        if (Object.keys(local).length > 0) {
          this.notes = local;
          this.log(`Migrated ${Object.keys(this.notes).length} files from local store`);
  ...
  private async saveNotes(): Promise<void> {
    ...
    /** 
      Strip transient notes and empty file entries before saving
      <a1exnd3r 2026-05-02 p:1>
    */
    for (const filePath of Object.keys(this.notes)) {
      this.notes[filePath] = this.notes[filePath]?.filter(n => !(n as any).__transient);
    ...
    if (this.globalNotesFilePath) {
      try {
        ...
        vscode.window.showErrorMessage(`NoteStack: Failed to save to global store: ${this.globalNotesFilePath}`);
      }
    }
  ...
  updateContextMenuState(editor: vscode.TextEditor | undefined): void {
    if (!editor) {
      vscode.commands.executeCommand('setContext', 'noteStack.hasNoteAtLine', false);
      return;
    }
    ...
    vscode.commands.executeCommand('setContext', 'noteStack.hasNoteAtLine', has);
  }
  ...
  updateDecorations(editor: vscode.TextEditor): void {
    ...
    const rel       = this.getRelativeFilePath(editor.document.fileName);
    const fileNotes = this.notes[rel] ?? this.findNotesByAbsPath(editor.document.fileName);
    ...
    for (const n of fileNotes) {
      ...
      if (drifted) {
        ...
        md.appendMarkdown(`⚠️ **Note drifted** — last anchor:\n${codeBlockMarkdown(n.anchor ?? '')}`);

        if (candidates.length === 0) {
          md.appendMarkdown(`*Anchor not found in document.*\n\n`);
        } 
        else if (candidates[0].exactMatch) {
          const args = encodeURIComponent(JSON.stringify([n.id, candidates[0].line]));
          const cmd  = `command:noteStack.reanchor?${args}`;
          md.appendMarkdown(`✅ **Exact match found at line ${candidates[0].line + 1}**\n${codeBlockMarkdown(candidates[0].text)}\n\n[**Re-anchor**](${cmd})\n\n`);
        } 
        else {
          md.appendMarkdown(`**Possible re-anchor positions:**\n\n`);
          for (const c of candidates) {
            const pct = Math.round(c.similarity * 100);
            md.appendMarkdown(`→ **Line ${c.line + 1}** *(${pct}% match)*\n${codeBlockMarkdown(c.text)}`);
          }
        }
      }

      if (n.priority) {
        md.appendMarkdown(`${priorityBadge(n.priority)}  ${author}*${timestamp}* (${daysLabel})`);
      } else {
      ...
      const endChar = n.textSel?.endChar  ?? (editor.document.lineAt(endLine).text.length || 1);
      ranges.push({ range: new vscode.Range(n.line, 0, endLine, endChar), hoverMessage: md });
    }

    editor.setDecorations(this.gutterDecorationType, ranges);
    editor.setDecorations(this.decorationType,       ranges.map(r => ({ range: r.range })));
  }

  updateStatusBar(editor?: vscode.TextEditor): void {
    const global      = this.readGlobalStore();
    const globalTotal = Object.values(global).flatMap(s => Object.values(s)).flat().length;
    const localTotal  = Object.values(this.notes).reduce((s, n) => s + (n?.length ?? 0), 0);

    const rel        = editor ? this.getRelativeFilePath(editor.document.fileName) : '';
    const fileNotes  = editor ? ((this.notes[rel] ?? this.findNotesByAbsPath(editor.document.fileName)) ?? []) : [];
    const fileCount  = fileNotes.length;

    const filePart   = fileCount > 0 ? `$(note) ${fileCount} · ` : '$(note) ';
    this.statusBarItem.text    = `${filePart}${localTotal} local · ${globalTotal} global`;
    this.statusBarItem.tooltip = 'NoteStack — click to open browser';
  }
  ...
  async addNote(): Promise<void> {
    ...
    const totalNotes = Object.values(this.notes).reduce((sum, n) => sum + (n?.length ?? 0), 0);
    ...
    this.notes[rel].push({
      ...
      hostName:  HOST_NAME,
      anchor: editor.document.lineAt(!editor.selection.isEmpty ? editor.selection.start.line : pos.line).text.trim() || undefined,
  ...
  async removeNote(): Promise<void> {
    ...
    const pos = editor.selection.active;
    const rel = this.getRelativeFilePath(editor.document.fileName);
    if (!this.notes[rel]) { vscode.window.showWarningMessage('No notes for this file'); return; }
  ...
  async showNotes(): Promise<void> {
    ...
    const picked = await vscode.window.showQuickPick(items, { placeHolder: 'Select a note to navigate to' });
    if (picked) {
  ...
  async showAllNotes(): Promise<void> {
    type AllNoteItem = vscode.QuickPickItem & { filePath: string; note: NoteEntry };
    ...
    if (items.length === 0) { vscode.window.showInformationMessage('No notes in workspace'); return; }
    ...
    if (picked) await this.openNote({ filePath: picked.filePath, fileName: path.basename(picked.filePath), note: picked.note });
  }

  async openNote(item: { filePath: string; fileName: string; note: NoteEntry }): Promise<void> {
    const root     = this.getWorkspaceRoot();
    const absPath  = path.isAbsolute(item.filePath) ? item.filePath : path.join(root, item.filePath);
    const cleanPath = absPath.includes('|') ? absPath.split('|').pop()! : absPath;

    try {
      const doc    = await vscode.workspace.openTextDocument(cleanPath);
      const editor = await vscode.window.showTextDocument(doc, { preserveFocus: false, preview: false, viewColumn: vscode.ViewColumn.One });
      ...
      /**
        ...
        Fix: Mark items __transient so saveNotes() strips it before writing to disk.

        <a1exnd3r 2026-05-02 p:1>
      */
      const rel = this.getRelativeFilePath(cleanPath);
      if (!this.notes[rel]) this.notes[rel] = [];
      if (!this.notes[rel].some(n => n.line === item.note.line && n.machineId === item.note.machineId)) {
      ...
      // Write once
      fs.writeFileSync(this.globalNotesFilePath, JSON.stringify(globalStore, null, 2));
      ...
      if (editor) this.updateDecorations(editor);
      if (this.browserPanel) {
        try { this.browserPanel.update(this); } catch { /* disposing */ }
      }

      // Open the newly anchored file
      await this.openNote({ ...item, filePath: newRel, fileName: path.basename(newRel) });
    }
  }

  async editNote(item: { filePath: string; fileName: string; note: NoteEntry }): Promise<void> {
    ...
    const localRel = this.notes[item.filePath] !== undefined
      ? item.filePath
      : this.getRelativeFilePath(item.filePath);

    if (this.notes[localRel]) {
      const idx = this.notes[localRel].findIndex(
        n => n.id === item.note.id || (n.line === item.note.line && n.character === item.note.character)
      );
      if (idx === -1) { vscode.window.showWarningMessage('Note not found'); return; }
    ...
    for (const [wsKey, wsStore] of Object.entries(store)) {
      for (const [filePath, notes] of Object.entries(wsStore)) {
        const idx = notes.findIndex(
          n => n.id === item.note.id || (n.line === item.note.line && n.character === item.note.character && filePath === item.filePath)
        );
        if (idx !== -1) {
  ...
  async editNote_deprecated(item: { filePath: string; fileName: string; note: NoteEntry }): Promise<void> {
    const rel = this.notes[item.filePath] !== undefined
    ? item.filePath
    : this.getRelativeFilePath(item.filePath);
    const locationLabel = `${item.fileName} · Line ${item.note.line + 1}, Col ${item.note.character + 1}`;
    ...
    if (!this.notes[rel]) { vscode.window.showWarningMessage('Note not found'); return; }
  ...
  async moveNote(): Promise<void> {
    ...
    if (!note) { vscode.window.showWarningMessage('No note on this line'); return; }

    this.noteInFlight = { filePath: rel, note };
    await vscode.commands.executeCommand('setContext', 'noteStack.noteInFlight', true);
  ...
  async reanchorNote(noteId: string, newLine: number): Promise<void> {
      ...
      for (const notes of Object.values(wsStore)) {
        ...
        if (n) {
          ...
          if (n.textSel) {
            ...
            const safeEndLine  = Math.min(newEndLine, editor.document.lineCount - 1);
            n.textSel = {
    ...
    if (!found) { 
      vscode.window.showWarningMessage('NoteStack: Note not found for re-anchor');
      return; 
    }
  ...
  async placeNote(): Promise<void> {
    const editor = vscode.window.activeTextEditor;
    if (!editor) return;
    if (!this.noteInFlight) { vscode.window.showWarningMessage('No note in flight'); return; }
    ...
    this.notes[targetRel] = this.notes[targetRel].filter(n => n.line !== pos.line);
    ...
    await vscode.commands.executeCommand('setContext', 'noteStack.noteInFlight', false);
    await this.saveNotes();
    this.lastCursorPosition = undefined;
    this.updateContextMenuState(editor);
    this.updateDecorations(editor);
  ...
  private cancelMove(): void {
    this.noteInFlight = undefined;
    vscode.commands.executeCommand('setContext', 'noteStack.noteInFlight', false);
  }

  async deleteNote(item: { filePath: string; fileName: string; note: NoteEntry }): Promise<void> {
    ...
    const before = this.notes[rel].length;
    this.notes[rel] = this.notes[rel].filter(
      n => !(n.line === item.note.line && n.character === item.note.character)
    );
    if (this.notes[rel].length === before) { vscode.window.showWarningMessage('Note not found'); return; }
  ...
  async getWorkspaceStats(): Promise<{ totalNotes: number; totalFiles: number }> {
    let totalNotes = 0, totalFiles = 0;
    for (const notes of Object.values(this.notes)) {
      if (notes?.length) { totalFiles++; totalNotes += notes.length; }
    }
    return { totalNotes, totalFiles };
  }

  openRefUrl(item: { filePath: string; fileName: string; note: NoteEntry }): void {
  ...
  async importNotes(): Promise<void> {
    ...
    try {
      incoming = this.sanitizeStore(JSON.parse(fs.readFileSync(uris[0].fsPath, 'utf8')));
    } catch (e) {
      vscode.window.showErrorMessage(`NoteStack: Failed to parse import file: ${e}`);
      return;
    }
    ...
    fs.writeFileSync(this.globalNotesFilePath, JSON.stringify(store, null, 2));
    this.treeDataProvider.refresh();
    if (this.browserPanel) try { this.browserPanel.update(this); } catch { /* disposing */ }
    vscode.window.showInformationMessage(`NoteStack: Imported ${imported} note(s).`);
  }
  ...
  async exportNotes(): Promise<void> {
    ...
    const uri = await vscode.window.showSaveDialog({
      defaultUri: vscode.Uri.file(path.join(this.getWorkspaceRoot() || '', 'notestack-export.json')),
      filters: { 'NoteStack Export': ['json'] },
    ...
    vscode.window.showInformationMessage(`NoteStack: Exported ${Object.keys(exportStore).length} workspace(s) to ${path.basename(uri.fsPath)}`);
  }
  ...
  async exportToMarkdown(): Promise<void> {
    ...
    for (const [wsKey, store] of Object.entries(global)) {
      ...
      for (const [filePath, notes] of Object.entries(store)) {
        ...
        const sorted = [...notes].sort((a, b) => prioritySort(a.priority, b.priority) || a.line - b.line);
    ...
    const defaultUri = vscode.Uri.file(
      path.join(this.getWorkspaceRoot() || process.env.HOME || '', `note-stack-export_${dateStr}_${APP_ID}.md`)
    );
    ...
    try {
      ...
      vscode.window.showErrorMessage(`NoteStack: Export failed: ${saveUri.fsPath}`);
    }
  }
