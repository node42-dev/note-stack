...
export function getGitBlameDateForLine(filePath: string, line: number): string | undefined {
  try {
    const cwd = path.dirname(filePath);
    const fileName = path.basename(filePath);
    const gitLine = line + 1; // git blame is 1-indexed
    const output = execSync(
      `git blame -L ${gitLine},${gitLine} --porcelain -- "${fileName}"`,
      { cwd, encoding: 'utf8', stdio: 'pipe' },
    );
    const match = output.match(/^author-time (\d+)$/m);
