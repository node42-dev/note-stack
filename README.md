...
| **Code Tags Browser**  | Full-panel view of every code tag across the workspace — search, sort, priority and keyword filters, inline code preview on hover |
| **Blame Drift**        | Optionally compares a code tag's stamped date against the `git blame` date for that line, flagging tags whose comment has gone stale |
...
| `noteStack.codeTagKeywords` | `["ISSUE", ...]` | Keywords scanned for in comments and shown in the Code Tags tree. Case-insensitive. Changes trigger an automatic re-scan. |
| `noteStack.codeTagBlameDrift` | `false` | Compare each code tag's date against the git blame date for that line, surfaced as drift in the tag tooltip and Code Tags Browser. Runs `git blame` per tagged line during scan, so it's opt-in. |
...
| Open Code Tags Browser | — | Full-panel view of every code tag in the workspace |
